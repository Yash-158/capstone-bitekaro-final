"""
BiteKaro — Recommendation API (FastAPI)
========================================
Exposes one main endpoint:

    POST /recommend
    {
        "customer_id":  "C00042",    // null = new/guest customer
        "cart_items":   ["HB02"],    // item IDs currently in cart
        "mood":         "tired",     // happy | tired | thirsty | hungry
        "hour":         14,          // 0-23 (server fills this if null)
        "month":        6,           // 1-12 (server fills this if null)
        "top_k":        5            // how many recs to return
    }

Run:
    pip install fastapi uvicorn
    uvicorn serve:app --host 0.0.0.0 --port 5000 --reload
"""

import sys
import os

# ── Fix path for flat folder structure ───────────────────────────────────────
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from datetime import datetime
import numpy as np
import joblib
from typing import Optional, List

# ── FastAPI import ────────────────────────────────────────────────────────────
try:
    from fastapi import FastAPI
    from fastapi.middleware.cors import CORSMiddleware
    from pydantic import BaseModel
    HAS_FASTAPI = True
except ImportError:
    HAS_FASTAPI = False
    print("FastAPI not installed. Run: pip install fastapi uvicorn")

from menu import ITEM_IDS, ITEM_LOOKUP, IDX_TO_ID, ID_TO_IDX

# ── Paths ─────────────────────────────────────────────────────────────────────
BASE   = os.path.dirname(os.path.abspath(__file__))
MODELS = os.path.join(BASE, "models")

# ── Load models ───────────────────────────────────────────────────────────────
print("Loading models...")
assoc_data = joblib.load(os.path.join(MODELS, "association_rules.pkl"))
svd_data   = joblib.load(os.path.join(MODELS, "svd_model.pkl"))
pop_data   = joblib.load(os.path.join(MODELS, "popularity_baseline.pkl"))
print("All models loaded")

RULES_LOOKUP = assoc_data["rules_lookup"]
POP_OVERALL  = pop_data["overall"]
POP_TIMESLOT = pop_data["by_time_slot"]
POP_SEASON   = pop_data["by_season"]

USER_FACTORS = svd_data["user_factors"]
ITEM_FACTORS = svd_data["item_factors"]
CUST_TO_IDX  = svd_data["cust_to_idx"]


# ═══════════════════════════════════════════════════════════════════════════════
#  HELPER FUNCTIONS
# ═══════════════════════════════════════════════════════════════════════════════

def get_season(month: int) -> str:
    if month in [3, 4, 5, 6]: return "summer"
    if month in [7, 8, 9]:    return "monsoon"
    if month in [10]:          return "autumn"
    return "winter"

def get_time_slot(hour: int) -> str:
    if hour < 11:  return "morning"
    if hour < 15:  return "lunch"
    if hour < 19:  return "afternoon"
    return "evening"


# ═══════════════════════════════════════════════════════════════════════════════
#  MOOD BOOST CONFIG
# ═══════════════════════════════════════════════════════════════════════════════

MOOD_BOOSTS = {
    "tired": {
        "tags":       ["energizing", "strong", "coffee"],
        "categories": ["hot_beverages"],
        "multiplier": 1.6,
    },
    "thirsty": {
        "tags":       ["refreshing", "cold", "light"],
        "categories": ["cold_beverages"],
        "multiplier": 1.7,
    },
    "hungry": {
        "tags":       ["filling", "meal", "heavy"],
        "categories": ["meals", "snacks"],
        "multiplier": 1.5,
    },
    "happy": {
        "tags":       ["popular", "sweet", "indulgent"],
        "categories": ["desserts", "cold_beverages"],
        "multiplier": 1.3,
    },
}


# ═══════════════════════════════════════════════════════════════════════════════
#  CONTEXT LAYER
# ═══════════════════════════════════════════════════════════════════════════════

def apply_context_boost(base_scores: dict, mood: str, hour: int,
                         month: int, cart_items: list) -> dict:
    season    = get_season(month)
    time_slot = get_time_slot(hour)
    scores    = dict(base_scores)

    cart_cats  = [ITEM_LOOKUP[i]["category"] for i in cart_items if i in ITEM_LOOKUP]
    bev_count  = cart_cats.count("cold_beverages") + cart_cats.count("hot_beverages")
    food_count = cart_cats.count("meals") + cart_cats.count("snacks")

    for item_id, score in scores.items():
        item = ITEM_LOOKUP.get(item_id)
        if not item:
            continue

        tags = item["tags"]
        cat  = item["category"]

        # Mood boost
        if mood and mood in MOOD_BOOSTS:
            mood_cfg = MOOD_BOOSTS[mood]
            if cat in mood_cfg["categories"] or any(t in tags for t in mood_cfg["tags"]):
                score *= mood_cfg["multiplier"]

        # Season boost
        season_score = POP_SEASON.get(season, {}).get(item_id, 0.5)
        score       *= (0.7 + 0.6 * season_score)

        # Time-slot boost
        ts_score = POP_TIMESLOT.get(time_slot, {}).get(item_id, 0.5)
        score   *= (0.8 + 0.4 * ts_score)

        # Suppress over-recommended categories
        if bev_count >= 2 and cat in ["cold_beverages", "hot_beverages"]:
            score *= 0.3
        if food_count >= 2 and cat == "meals":
            score *= 0.4

        # Suppress cart items
        if item_id in cart_items:
            score = -999

        scores[item_id] = float(score)

    return scores


# ═══════════════════════════════════════════════════════════════════════════════
#  REASON STRING
# ═══════════════════════════════════════════════════════════════════════════════

def get_reason(item_id, item, mood, hour, month, cart_items):
    season    = get_season(month)
    time_slot = get_time_slot(hour)
    tags      = item["tags"]

    if mood == "tired"   and ("energizing" in tags or "coffee" in tags):
        return "Great pick when you need an energy boost"
    if mood == "thirsty" and item["category"] == "cold_beverages":
        return "Perfect to quench your thirst"
    if mood == "hungry"  and ("filling" in tags or "meal" in tags):
        return "This should hit the spot"
    if mood == "happy"   and ("sweet" in tags or "popular" in tags):
        return "A fan favourite for happy moments"
    if season == "summer" and "cold" in tags:
        return "Cool down with this summer favourite"
    if season in ["winter", "monsoon"] and "hot" in tags:
        return f"Perfect warm treat for {season}"
    if time_slot == "morning" and "breakfast" in tags:
        return "A great way to start your morning"
    if time_slot == "lunch" and "meal" in tags:
        return "Popular lunch choice"
    if "popular" in tags:
        return "One of our bestsellers"
    if any(
        c_item in RULES_LOOKUP and
        any(r["item_id"] == item_id for r in RULES_LOOKUP.get(c_item, []))
        for c_item in cart_items
    ):
        return "Pairs perfectly with your order"
    return "You might enjoy this"


# ═══════════════════════════════════════════════════════════════════════════════
#  MAIN RECOMMENDATION FUNCTION
# ═══════════════════════════════════════════════════════════════════════════════

def get_recommendations(customer_id, cart_items, mood, hour, month, top_k=5):

    is_returning  = customer_id and customer_id in CUST_TO_IDX
    customer_type = "returning" if is_returning else "new"

    # Base scores
    if is_returning:
        u_idx       = CUST_TO_IDX[customer_id]
        user_vec    = USER_FACTORS[u_idx]
        raw_scores  = ITEM_FACTORS @ user_vec
        base_scores = {ITEM_IDS[i]: float(raw_scores[i]) for i in range(len(ITEM_IDS))}
        model_used  = "svd"
    else:
        base_scores = dict(POP_OVERALL)
        model_used  = "popularity"

    # Apply context
    adjusted = apply_context_boost(base_scores, mood, hour, month, cart_items)

    # Rank
    ranked   = sorted(adjusted.items(), key=lambda x: x[1], reverse=True)
    ranked   = [(iid, sc) for iid, sc in ranked if iid not in cart_items]
    top_items = ranked[:top_k]

    # Association rule injections for cart items
    assoc_injections = []
    for cart_item in cart_items:
        if cart_item in RULES_LOOKUP:
            for rule in RULES_LOOKUP[cart_item][:2]:
                if rule["item_id"] not in cart_items:
                    assoc_injections.append((rule["item_id"], rule["lift"] * 1.5))

    if assoc_injections:
        existing_ids = [i[0] for i in top_items]
        injected     = []
        for iid, boost in assoc_injections[:2]:
            if iid not in existing_ids:
                injected.append((iid, boost))
        top_items = injected + top_items
        top_items = list(dict.fromkeys([i[0] for i in top_items]))[:top_k]
        top_items = [(iid, adjusted.get(iid, 0)) for iid in top_items]

    # Build response
    recommendations = []
    for item_id, score in top_items[:top_k]:
        item = ITEM_LOOKUP.get(item_id)
        if not item:
            continue

        source = "association" if any(item_id == inj[0] for inj in assoc_injections[:2]) \
                 else ("svd" if is_returning else "popularity")

        recommendations.append({
            "item_id":   item_id,
            "item_name": item["name"],
            "category":  item["category"],
            "price":     item["price"],
            "score":     round(min(max(score, 0), 1), 4),
            "reason":    get_reason(item_id, item, mood, hour, month, cart_items),
            "source":    source,
        })

    return {
        "recommendations": recommendations,
        "model_used":      model_used,
        "customer_type":   customer_type,
    }


# ═══════════════════════════════════════════════════════════════════════════════
#  FASTAPI APP
# ═══════════════════════════════════════════════════════════════════════════════

if HAS_FASTAPI:
    app = FastAPI(
        title="BiteKaro Recommendation API",
        description="AI-powered food recommendations for BiteKaro kiosk",
        version="1.0.0"
    )

    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
        allow_headers=["*"],
    )

    class RecommendRequest(BaseModel):
        customer_id: Optional[str] = None
        cart_items:  List[str]     = []
        mood:        Optional[str] = "happy"
        hour:        Optional[int] = None
        month:       Optional[int] = None
        top_k:       Optional[int] = 5

    @app.get("/health")
    def health():
        return {
            "status":        "ok",
            "models_loaded": True,
            "n_items":       len(ITEM_IDS),
        }

    @app.post("/recommend")
    def recommend(req: RecommendRequest):
        now   = datetime.now()
        hour  = req.hour  if req.hour  is not None else now.hour
        month = req.month if req.month is not None else now.month
        return get_recommendations(
            customer_id = req.customer_id,
            cart_items  = req.cart_items or [],
            mood        = req.mood or "happy",
            hour        = hour,
            month       = month,
            top_k       = req.top_k or 5,
        )

    @app.get("/items")
    def list_items():
        from menu import ALL_ITEMS
        return {"items": ALL_ITEMS}


# ═══════════════════════════════════════════════════════════════════════════════
#  CLI TEST
# ═══════════════════════════════════════════════════════════════════════════════

if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("  BiteKaro Recommendation API - CLI Test")
    print("=" * 60)

    test_cases = [
        {
            "label":       "New customer, tired mood, morning",
            "customer_id": None,
            "cart_items":  [],
            "mood":        "tired",
            "hour":        9,
            "month":       3,
        },
        {
            "label":       "Returning customer, cart has Samosa, lunch time",
            "customer_id": "C00001",
            "cart_items":  ["SN01"],
            "mood":        "hungry",
            "hour":        13,
            "month":       2,
        },
        {
            "label":       "Guest customer, thirsty, summer afternoon",
            "customer_id": None,
            "cart_items":  [],
            "mood":        "thirsty",
            "hour":        16,
            "month":       5,
        },
        {
            "label":       "Returning customer, cart has Cappuccino, happy",
            "customer_id": "C00042",
            "cart_items":  ["HB02"],
            "mood":        "happy",
            "hour":        15,
            "month":       12,
        },
    ]

    for tc in test_cases:
        print(f"\n  Test: {tc['label']}")
        result = get_recommendations(
            customer_id = tc["customer_id"],
            cart_items  = tc["cart_items"],
            mood        = tc["mood"],
            hour        = tc["hour"],
            month       = tc["month"],
        )
        print(f"  Model: {result['model_used']} | Customer: {result['customer_type']}")
        print(f"  {'Item':<22} {'Category':<18} {'Price':>7}  Reason")
        print(f"  {'-'*22}  {'-'*18}  {'-'*7}  {'-'*30}")
        for rec in result["recommendations"]:
            print(f"  {rec['item_name']:<22}  {rec['category']:<18}  "
                  f"Rs{rec['price']:>5}  {rec['reason']}")

    print("\n" + "=" * 60)
    print("  API test complete. To run as a server:")
    print("  pip install fastapi uvicorn")
    print("  uvicorn serve:app --host 0.0.0.0 --port 5000 --reload")
    print("=" * 60)